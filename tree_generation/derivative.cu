...
static __device__ __forceinline__ void maybe_print_site_deriv_debug(
    int enabled,
    int debug_target_id,
...
void evaluate_site_derivatives_general(
    ...
    size_t site,
    fp_t* placement_clv, 
    int site_debug_enabled,
    int site_debug_target_id,
    int site_debug_offset,
    int site_debug_limit,
    int site_debug_iter,
    const char* site_debug_kernel_name,
    int target_id,
    int iter,
    double& d1_out,
    double& d2_out)
{
    ...
    maybe_print_site_deriv_debug(
        site_debug_enabled,
        site_debug_target_id,
        site_debug_offset,
        site_debug_limit,
        site_debug_iter,
        site_debug_kernel_name,
        target_id,
        iter,
        site,
        site_lk0,
    ...
    double& d2_out)
{
    ...
    for (int r = 0; r < RATE_CATS; ++r) {
        ...
        if (site_debug_enabled &&
            (site_debug_target_id < 0 || target_id == site_debug_target_id) &&
            iter == site_debug_iter &&
            static_cast<int>(site) >= site_debug_offset &&
            static_cast<int>(site) < site_debug_offset + site_debug_limit &&
            site_debug_kernel_name &&
            site_debug_kernel_name[0] == 'p') {
            printf("[site-rate-debug] kernel=%s target=%d iter=%d site=%llu rate=%d\n",
                   site_debug_kernel_name,
                   target_id,
                   iter,
                   static_cast<unsigned long long>(site),
                   r);
            printf("[site-rate-debug] kernel=%s target=%d iter=%d site=%llu rate=%d sum=(%.12e,%.12e,%.12e,%.12e)\n",
                   site_debug_kernel_name,
                   target_id,
                   iter,
                   static_cast<unsigned long long>(site),
                   r,
                   static_cast<double>(sum_row[0]),
                   static_cast<double>(sum_row[1]),
                   static_cast<double>(sum_row[2]),
                   static_cast<double>(sum_row[3]));
            printf("[site-rate-debug] kernel=%s target=%d iter=%d site=%llu rate=%d diag0=(%.12e,%.12e,%.12e,%.12e)\n",
                   site_debug_kernel_name,
                   target_id,
                   iter,
                   static_cast<unsigned long long>(site),
                   r,
                   static_cast<double>(diag_row[0]),
                   static_cast<double>(diag_row[4]),
                   static_cast<double>(diag_row[8]),
                   static_cast<double>(diag_row[12]));
            printf("[site-rate-debug] kernel=%s target=%d iter=%d site=%llu rate=%d diag1=(%.12e,%.12e,%.12e,%.12e)\n",
                   site_debug_kernel_name,
                   target_id,
                   iter,
                   static_cast<unsigned long long>(site),
                   r,
                   static_cast<double>(diag_row[1]),
                   static_cast<double>(diag_row[5]),
                   static_cast<double>(diag_row[9]),
                   static_cast<double>(diag_row[13]));
            printf("[site-rate-debug] kernel=%s target=%d iter=%d site=%llu rate=%d diag2=(%.12e,%.12e,%.12e,%.12e) cat=(%.12e,%.12e,%.12e)\n",
                   site_debug_kernel_name,
                   target_id,
                   iter,
                   static_cast<unsigned long long>(site),
                   r,
                   static_cast<double>(diag_row[2]),
                   static_cast<double>(diag_row[6]),
                   static_cast<double>(diag_row[10]),
                   static_cast<double>(diag_row[14]),
                   static_cast<double>(cat0),
                   static_cast<double>(cat1),
                   static_cast<double>(cat2));
        }
        ...
        switch (D.rate_cats) {
            case 1:
                accumulate_site_derivatives_states4_noinv<1, WRITE_PLACEMENT_CLV>(
                    D, sumtable_op, diag_shared, pattern_weights, placement_clv, tid, step, site_debug_enabled, site_debug_target_id, site_debug_offset, site_debug_limit, site_debug_iter, site_debug_kernel_name, target_id, iter, df_local, ddf_local);
                return;
            case 4:
