...
{
  const float * left_matrix = partition->pmatrix[op->child1_matrix_index];
  const float * right_matrix = partition->pmatrix[op->child2_matrix_index];
  float * parent_clv = partition->clv[op->parent_clv_index];
                            ...
                            const pll_operation_t * op)
{
  ...
  const float * right_matrix = partition->pmatrix[op->child2_matrix_index];
  float * parent_clv = partition->clv[op->parent_clv_index];
  float * left_clv = partition->clv[op->child1_clv_index];
  float * right_clv = partition->clv[op->child2_clv_index];
  unsigned int * parent_scaler;
                            ...
                            const pll_operation_t * op)
{
  ...
  float * bclv_buffer = partition->repeats ? partition->repeats->bclv_buffer : 0;;
