...
static int update_charmap(pll_partition_t * partition, const pll_state_t * map)
{
  ...
  if (new_states_count)
  {
    ...
    unsigned int l2_maxstates = (unsigned int)ceilf(log2f(partition->maxstates));
    ...
    partition->ttlookup = pll_aligned_alloc(alloc_size * sizeof(float),
                                            partition->alignment);
    if (!partition->ttlookup)
    {
                                                  ...
                                                  unsigned int attributes)
{
  ...
  /* clv */
  partition->clv = (float **)calloc(partition->nodes, sizeof(float *));
  if (!partition->clv)
  {
    dealloc_partition_data(partition);
    pll_errno = PLL_ERROR_MEM_ALLOC;
  ...
  if (!pll_repeats_enabled(partition)) 
  {
    ...
    for (i = start; i < partition->tips + partition->clv_buffers; ++i)
    {
      ...
      memset(partition->clv[i],
             0,
             (size_t)sites_alloc*states_padded*rate_cats*sizeof(float));
    }
  }
  /* pmatrix */
  partition->pmatrix = (float **)calloc(partition->prob_matrices,
                                         sizeof(float *));
  if (!partition->pmatrix)
  {
    dealloc_partition_data(partition);
  ...
  /* allocate transition probability matrices in contiguous space, in order
     ...
     of states_padded. */
  size_t displacement = (states_padded - states) * (states_padded) * sizeof(float);
  partition->pmatrix[0] = pll_aligned_alloc(partition->prob_matrices *
                                            states * states_padded * rate_cats *
                                            sizeof(float) + displacement,
                                            partition->alignment);
  ...
  memset(partition->pmatrix[0],0,partition->prob_matrices * states *
                                 states_padded * rate_cats * sizeof(float) +
                                 displacement);

  /* eigenvecs */
  partition->eigenvecs = (float **)calloc(partition->rate_matrices,
                                           sizeof(float *));
  if (!partition->eigenvecs)
  {
    dealloc_partition_data(partition);
  ...
  for (i = 0; i < partition->rate_matrices; ++i)
  {
    ...
    memset(partition->eigenvecs[i], 0, states * states_padded * sizeof(float));
    /* TODO: don't forget to add code for SSE/AVX */
  }

  /* inv_eigenvecs */
  partition->inv_eigenvecs = (float **)calloc(partition->rate_matrices,
                                               sizeof(float *));
  if (!partition->inv_eigenvecs)
  {
    dealloc_partition_data(partition);
  ...
  for (i = 0; i < partition->rate_matrices; ++i)
  {
    ...
    memset(partition->inv_eigenvecs[i], 0, states * states_padded * sizeof(float));
    /* TODO: don't forget to add code for SSE/AVX */
  }

  /* eigenvals */
  partition->eigenvals = (float **)calloc(partition->rate_matrices,
                                               sizeof(float *));
  if (!partition->eigenvals)
  {
    dealloc_partition_data(partition);
  ...
  for (i = 0; i < partition->rate_matrices; ++i)
  {
    partition->eigenvals[i] = pll_aligned_alloc(states_padded*sizeof(float),
                                                partition->alignment);
    ...
    memset(partition->eigenvals[i], 0, states_padded * sizeof(float));
    /* TODO: don't forget to add code for SSE/AVX */
  }

  /* subst_params */
  partition->subst_params = (float **)calloc(partition->rate_matrices,
                                              sizeof(float *));
  if (!partition->subst_params)
  {
    dealloc_partition_data(partition);
  ...
  /* frequencies */
  partition->frequencies = (float **)calloc(partition->rate_matrices,
                                             sizeof(float *));
  if (!partition->frequencies)
  {
    dealloc_partition_data(partition);
  ...
  for (i = 0; i < partition->rate_matrices; ++i)
  {
    partition->frequencies[i] = pll_aligned_alloc(states_padded*sizeof(float),
                                                  partition->alignment);
    ...
    memset(partition->frequencies[i],
           0,
           states_padded*sizeof(float));
  }

  /* rates */
  partition->rates = (float *)calloc(partition->rate_cats,sizeof(float));
  if (!partition->rates)
  {
    dealloc_partition_data(partition);
    pll_errno = PLL_ERROR_MEM_ALLOC;
  ...
  /* rate weights */
  partition->rate_weights = (float *)calloc(partition->rate_cats,sizeof(float));
  ...
  partition->prop_invar = (float *)calloc(partition->rate_matrices,
                                             sizeof(float));
  if (!partition->prop_invar)
  {
    dealloc_partition_data(partition);
                     ...
                     const char * sequence)
{
  pll_state_t c;
  unsigned int i,j;
  float * tipclv = partition->clv[tip_index];
