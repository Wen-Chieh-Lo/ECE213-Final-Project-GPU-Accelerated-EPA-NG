...
PLL_EXPORT int pll_core_update_pmatrix_4x4_avx(float ** pmatrix,
                                               unsigned int rate_cats,
                                               const float * rates,
                                               const float * branch_lengths,
                                               const unsigned int * matrix_indices,
                                               const unsigned int * params_indices,
                                               const float * prop_invar,
                                               float * const * eigenvals,
                                               float * const * eigenvecs,
                                               float * const * inv_eigenvecs,
                                               unsigned int count)
{
  unsigned int i,n;
  float * expd;
  ...
  expd = (float *)pll_aligned_alloc(4*sizeof(float), PLL_ALIGNMENT_AVX);
  ...
  __m128 idm_row0 = _mm_set_ps(0.f, 0.f, 0.f, 1.0f);
  __m128 idm_row1 = _mm_set_ps(0.f, 0.f, 1.0f, 0.f);
  __m128 idm_row2 = _mm_set_ps(0.f, 1.0f, 0.f, 0.f);
  __m128 idm_row3 = _mm_set_ps(1.0f, 0.f, 0.f, 0.f);

  for (i = 0; i < count; ++i)
  {
    assert(branch_lengths[i] >= 0);
    
    xmm3 = _mm_set1_ps(branch_lengths[i]);
    ...
    for (n = 0; n < rate_cats; ++n)
    {
      ...
      if (branch_lengths[i] > 0.)
      {
        ...
        xmm2 = _mm_load_ps(inv_evecs+0);
        xmm4 = _mm_load_ps(inv_evecs+4);
        xmm5 = _mm_load_ps(inv_evecs+8);
        xmm6 = _mm_load_ps(inv_evecs+12);

        xmm7 = _mm_mul_ps(xmm2,xmm1);       /* temp row 0 */
        xmm2 = _mm_mul_ps(xmm4,xmm1);       /* temp row 1 */
        xmm4 = _mm_mul_ps(xmm5,xmm1);       /* temp row 2 */
        xmm5 = _mm_mul_ps(xmm6,xmm1);       /* temp row 3 */

        /* transpose eigenvector */
        xmm6 = _mm_load_ps(evecs+0);
        xmm1 = _mm_load_ps(evecs+4);
        xmm8 = _mm_load_ps(evecs+8);
        xmm9 = _mm_load_ps(evecs+12);

        /* transpose eigenvectors */
        ymm0 = _mm_unpacklo_ps(xmm6,xmm1); // (64,64,64,64) b1,a1,b0,a0  
        ymm1 = _mm_unpackhi_ps(xmm6,xmm1); // (64,64,64,64) b3,a3,b2,a2  
        ymm2 = _mm_unpacklo_ps(xmm8,xmm9); // (64,64,64,64) b1,a1,b0,a0  
        ymm3 = _mm_unpackhi_ps(xmm8,xmm9); // (64,64,64,64) b3,a3,b2,a2  

        xmm6 = _mm_shuffle_ps(ymm0,ymm2, 64+4);            // (hi,lo) b0,a0 -> shuffle (01000100) 64+4
        xmm1 = _mm_shuffle_ps(ymm0,ymm2, 128+64+32+8+4+2); // (hi,lo) b1,a1 -> shuffle (11101110) 128+64+32+8+4+2
        xmm8 = _mm_shuffle_ps(ymm1,ymm3, 64+4);            // (hi,lo) b0,a0 -> shuffle (01000100) 64+4
        xmm9 = _mm_shuffle_ps(ymm1,ymm3, 128+64+32+8+4+2); // (hi,lo) b1,a1 -> shuffle (11101110) 128+64+32+8+4+2

        /* pmat row 0*/

        ymm0 = _mm_mul_ps(xmm7,xmm6);
        ymm1 = _mm_mul_ps(xmm7,xmm1);
        ymm2 = _mm_mul_ps(xmm7,xmm8);
        ymm3 = _mm_mul_ps(xmm7,xmm9);

        ymm0 = _mm_hadd_ps(ymm0,ymm1);
        ymm1 = _mm_hadd_ps(ymm2,ymm3);

        ymm0 = _mm_hadd_ps(ymm0,ymm1);

        ymm0 = _mm_add_ps(ymm0,idm_row0);
        _mm_store_ps(pmat+0,ymm0);

        /* pmat row 1 */

        ymm0 = _mm_mul_ps(xmm2,xmm6);
        ymm1 = _mm_mul_ps(xmm2,xmm1);
        ymm2 = _mm_mul_ps(xmm2,xmm8);
        ymm3 = _mm_mul_ps(xmm2,xmm9);
        ...
        ymm0 = _mm_add_ps(ymm0,idm_row1);
        _mm_store_ps(pmat+4,ymm0);

        /* pmat row 2 */

        ymm0 = _mm_mul_ps(xmm4,xmm6);
        ymm1 = _mm_mul_ps(xmm4,xmm1);
        ymm2 = _mm_mul_ps(xmm4,xmm8);
        ymm3 = _mm_mul_ps(xmm4,xmm9);
        ...
        ymm0 = _mm_add_ps(ymm0,idm_row2);
        _mm_store_ps(pmat+8,ymm0);

        /* pmat row 3 */

        ymm0 = _mm_mul_ps(xmm5,xmm6);
        ymm1 = _mm_mul_ps(xmm5,xmm1);
        ymm2 = _mm_mul_ps(xmm5,xmm8);
        ymm3 = _mm_mul_ps(xmm5,xmm9);
        ...
        ymm0 = _mm_add_ps(ymm0,idm_row3);
        _mm_store_ps(pmat+12,ymm0);
      }
      else
      {
        _mm_store_ps(pmat+0,xmm0);
        _mm_store_ps(pmat+4,xmm0);
        _mm_store_ps(pmat+8,xmm0);
        _mm_store_ps(pmat+12,xmm0);
