                                              ...
                                              unsigned int attrib)
{
  unsigned int i,j,k;
  float logl = 0;
  const float * freqs = NULL;

  float prop_invar = 0;

  float term, term_r;
  float site_lk, inv_site_lk;
  ...
  for (i = 0; i < sites; ++i)
  {
    ...
    site_lk = logf(site_lk);
    if (scaler && scaler[i])
      site_lk += scaler[i] * logf(PLL_SCALE_THRESHOLD);
...
PLL_EXPORT float pll_core_root_loglikelihood_repeats_generic(unsigned int states,
                                                              unsigned int sites,
                                                              unsigned int rate_cats,
                                                              const float * clv,
                                                              ...
                                                              float * persite_lnl)
{
  ...
  for (i = 0; i < sites; ++i)
  {
    ...
    site_lk = logf(site_lk);
    if (scaler && scaler[id])
      site_lk += scaler[id] * logf(PLL_SCALE_THRESHOLD);
...
PLL_EXPORT float pll_core_root_loglikelihood_repeats(unsigned int states,
                                                      ...
                                                      unsigned int attrib)
{
  float (*core_root_loglikelihood) (unsigned int states,
                                     unsigned int sites,
                                     unsigned int rate_cats,
                                     const float * clv,
                                     const unsigned int * site_id,
...
PLL_EXPORT
float pll_core_edge_loglikelihood_ti_4x4(unsigned int sites,
                                          unsigned int rate_cats,
                                          ...
                                          unsigned int attrib)
{
  ...
  const float * clvp = parent_clv;
  float prop_invar = 0;
  const float * pmat;
  const float * freqs = NULL;

  float terma, terma_r, termb, terminv;
  float site_lk, inv_site_lk;
  ...
  float scale_minlh[PLL_SCALE_RATE_MAXDIFF];
  ...
  for (n = 0; n < sites; ++n)
  {
    ...
    if (site_scalings)
    {
      if (terminv > 0.)
      {
        ...
        float scale_factor = scale_minlh[capped_scalings-1];
        site_lk = logf(terma * scale_factor + terminv);
      }
      else
      {
        site_lk = logf(terma);
        site_lk += site_scalings * logf(PLL_SCALE_THRESHOLD);
      }
    }
    else
    {
      site_lk = logf(terma + terminv);
    }
...
PLL_EXPORT
float pll_core_edge_loglikelihood_ti(unsigned int states,
                                      unsigned int sites,
                                      unsigned int rate_cats,
                                      const float * parent_clv,
                                      const unsigned int * parent_scaler,
...
PLL_EXPORT
float pll_core_edge_loglikelihood_repeats(unsigned int states,
                                           unsigned int sites,
                                           ...
                                           unsigned int attrib)
{
  float (*core_edge_loglikelihood)(unsigned int states,
                                    unsigned int sites,
                                    const unsigned int child_sites,
                                    unsigned int rate_cats,
                                    const float * parent_clv,
...
PLL_EXPORT
float pll_core_edge_loglikelihood_repeats_generic(unsigned int states,
                                                   unsigned int sites,
                                                   const unsigned int child_sites,
                                                   unsigned int rate_cats,
                                                   const float * parent_clv,
                                                   ...
                                                   unsigned int attrib)
{
  ...
  for (n = 0; n < sites; ++n)
  {
    ...
    const float *clvp = &parent_clv[pid * span];
    const float *clvc = &child_clv[cid * span];
    if (per_rate_scaling)
    {
      /* compute minimum per-rate scaler -> common per-site scaler */
      site_scalings = UINT_MAX;
...
PLL_EXPORT
float pll_core_edge_loglikelihood_ii(unsigned int states,
                                      unsigned int sites,
                                      unsigned int rate_cats,
                                      const float * parent_clv,
                                      const unsigned int * parent_scaler,
