                                       ...
                                       unsigned int attrib)
{
  ...
  expd = (float *)malloc(states * sizeof(float));
  temp = (float *)malloc(states*states*sizeof(float));
    ...
    for (n = 0; n < rate_cats; ++n)
    {
      ...
      if (branch_lengths[i] > 0.f)
      {
        ...
        else
        {
          for (j = 0; j < states; ++j){
           expd[j] = expm1(evals[j] * rates[n] * branch_lengths[i]);
           //printf("%d,%d, %.10f,%.10f,%.10f,%.10f\n",n,j, expd[j],evals[j] , rates[n] , branch_lengths[i]);
          }
        }
        ...
        for (j = 0; j < states; ++j)
        {
          for (k = 0; k < states; ++k)
          {
            pmat[j*states_padded+k] = (j==k) ? 1.0f : 0;
            for (m = 0; m < states; ++m)
            {
