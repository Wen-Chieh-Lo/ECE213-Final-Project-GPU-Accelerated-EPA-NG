...
PLL_EXPORT void pll_core_update_partial_ii_4x4_avx_fp32_16dp(unsigned int sites,
                                                   unsigned int rate_cats,
                                                   float * parent_clv,
                                                   unsigned int * parent_scaler,
                                                   ...
                                                   unsigned int attrib)
{
  ...
  for (n = 0; n < sites; ++n)
  {
    ...
    {
      /* compute vector of x */
      xmm1 = _mm256_broadcast_ps(left_clv);
      xmm2 = _mm256_load_ps(lmat);
      xmm0 = _mm256_dp_ps(xmm1,xmm2, 128+64+32+16+1);

      ymm1 = _mm256_broadcast_ps(right_clv);
      ymm2 = _mm256_load_ps(rmat);
      ymm0 = _mm256_dp_ps(ymm1,ymm2, 128+64+32+16+1);

      lmat += 2*states;
      rmat += 2*states;

      xmm2 = _mm256_load_ps(lmat);
      xmm1 = _mm256_dp_ps(xmm1,xmm2, 128+64+32+16+4);

      ymm2 = _mm256_load_ps(rmat);
      ymm1 = _mm256_dp_ps(ymm1,ymm2, 128+64+32+16+4);

      lmat += 2*states;
      rmat += 2*states;

      xmm0 = _mm256_blend_ps(xmm0,xmm1,64+4);
      pmm0 = _mm256_set_epi32(7,5,3,1,6,2,4,0);
      xmm0 = _mm256_permutevar8x32_ps(xmm0,pmm0);

      ymm0 = _mm256_blend_ps(ymm0,ymm1,64+4);
      ymm0 = _mm256_permutevar8x32_ps(ymm0,pmm0);

      /* compute x*y */
      xmm128_0 = _mm_mul_ps(_mm256_extractf128_ps(xmm0,0), _mm256_extractf128_ps(ymm0,0));

      xmm1 = _mm256_broadcast_ps(left_clv+states);
      xmm2 = _mm256_load_ps(lmat);
      xmm0 = _mm256_dp_ps(xmm1,xmm2, 128+64+32+16+1);

      ymm1 = _mm256_broadcast_ps(right_clv+states);
      ymm2 = _mm256_load_ps(rmat);
      ymm0 = _mm256_dp_ps(ymm1,ymm2, 128+64+32+16+1);
      ...
      /* compute x*y */
      xmm128_1 = _mm_mul_ps(_mm256_extractf128_ps(xmm0,0), _mm256_extractf128_ps(ymm0,0));

            xmm1 = _mm256_broadcast_ps(left_clv+states*2);
      xmm2 = _mm256_load_ps(lmat);
      xmm0 = _mm256_dp_ps(xmm1,xmm2, 128+64+32+16+1);

      ymm1 = _mm256_broadcast_ps(right_clv+states*2);
      ymm2 = _mm256_load_ps(rmat);
      ymm0 = _mm256_dp_ps(ymm1,ymm2, 128+64+32+16+1);
      ...
      /* compute x*y */
      xmm128_2 = _mm_mul_ps(_mm256_extractf128_ps(xmm0,0), _mm256_extractf128_ps(ymm0,0));

      xmm1 = _mm256_broadcast_ps(left_clv+states*3);
      xmm2 = _mm256_load_ps(lmat);
      xmm0 = _mm256_dp_ps(xmm1,xmm2, 128+64+32+16+1);

      ymm1 = _mm256_broadcast_ps(right_clv+states*3);
      ymm2 = _mm256_load_ps(rmat);
      ymm0 = _mm256_dp_ps(ymm1,ymm2, 128+64+32+16+1);
      ...
      /* compute x*y */
      xmm128_3 = _mm_mul_ps(_mm256_extractf128_ps(xmm0,0), _mm256_extractf128_ps(ymm0,0));
      /* check if scaling is needed for the current rate category */
      /* check if scaling is needed for the current rate category */
      __m128 v_cmp = _mm_cmp_ps(xmm128_0, v_scale_threshold, _CMP_LT_OS);
      unsigned int rate_mask = _mm_movemask_ps(v_cmp);
      ...
      rate_mask = _mm_movemask_ps(v_cmp);
...
PLL_EXPORT void pll_core_update_partial_ii_4x4_avx_fp32_16(unsigned int sites,
                                                   unsigned int rate_cats,
                                                   float * parent_clv,
                                                   unsigned int * parent_scaler,
                                                   const float * left_clv,
                                                   ...
                                                   unsigned int attrib)
{
  ...
  for (n = 0; n < sites; ++n)
  {
    ...
    for (k = 0; k < rate_cats; k+=4)
    {
      ...
      /* compute vector of x */
      xmm1 = _mm256_broadcast_ps(left_clv+2*states);
      xmm2 = _mm256_load_ps(lmat);
      xmm0 = _mm256_dp_ps(xmm1,xmm2, 128+64+32+16+1);

      ymm1 = _mm256_broadcast_ps(right_clv+2*states);
      ymm2 = _mm256_load_ps(rmat);
      ymm0 = _mm256_dp_ps(ymm1,ymm2, 128+64+32+16+1);
      ...
      /* compute vector of x */
      xmm1 = _mm256_broadcast_ps(left_clv+3*states);
      xmm2 = _mm256_load_ps(lmat);
      xmm0 = _mm256_dp_ps(xmm1,xmm2, 128+64+32+16+1);

      ymm1 = _mm256_broadcast_ps(right_clv+3*states);
      ymm2 = _mm256_load_ps(rmat);
      ymm0 = _mm256_dp_ps(ymm1,ymm2, 128+64+32+16+1);
                                                ...
                                                unsigned int attrib)
{
  ...
  if (states == 4)
  {
    ...
    pll_core_update_partial_ii_4x4_avx_fp32(sites,
                                       rate_cats,
                                       parent_clv,
                                       parent_scaler,
                                       left_clv,
