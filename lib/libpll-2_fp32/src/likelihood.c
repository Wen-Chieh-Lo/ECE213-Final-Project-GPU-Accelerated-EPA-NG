...
static float edge_loglikelihood_tipinner(pll_partition_t * partition,
                                          unsigned int parent_clv_index,
...
static float edge_loglikelihood_asc_bias_ii(pll_partition_t * partition,
                                             const float * clvp,
                                             unsigned int * parent_scaler,
                                             ...
                                             const unsigned int * freqs_indices)
{
  ...
  float logl = 0;
  float terma, terma_r, termb;
  float site_lk;

  const float * freqs = NULL;
  const float * pmatrix = partition->pmatrix[matrix_index];
  unsigned int states = partition->states;
  unsigned int states_padded = partition->states_padded;
  unsigned int scale_factors;
  unsigned int * pattern_weights = partition->pattern_weights;
  float * rate_weights = partition->rate_weights;
...
static float edge_loglikelihood(pll_partition_t * partition,
                                 unsigned int parent_clv_index,
                                 int parent_scaler_index,
                                 unsigned int child_clv_index,
                                 ...
                                 float * persite_lnl)
{
  ...
  const float * clvp = partition->clv[parent_clv_index];
  const float * clvc = partition->clv[child_clv_index];
...
static float edge_loglikelihood_repeats(pll_partition_t * partition,
                                 unsigned int parent_clv_index,
                                 int parent_scaler_index,
                                 unsigned int child_clv_index,
                                 int child_scaler_index,
