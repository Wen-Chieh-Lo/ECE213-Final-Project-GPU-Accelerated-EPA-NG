...
static int mytqli(float *d, float *e, const unsigned int n, float **z)
{
      ...
      do
        {
          for (m = l; m <= n - 1; m++)
            {
              dd = fabsf(d[m - 1]) + fabsf(d[m]);
              if (fabsf(e[m - 1]) + dd == dd)
                break;
            }
          if (m != l)
           {
             ...
             r = sqrtf((g * g) + 1.0f);
             g = d[m - 1] - d[l - 1] + e[l - 1] / (g + ((g < 0)?-fabsf(r):fabsf(r)));/*MYSIGN(r, g));*/
             s = c = 1.0f;
             p = 0.0f;
...
static void mytred2(float **a, const unsigned int n, float *d, float *e)
{
                                   ...
                                   unsigned int states)
{
  ...
  float * params_normalized = (float *)malloc(sizeof(float) * params_count);
  if (!params_normalized)
    return NULL;

  memcpy(params_normalized,params,params_count*sizeof(float));

  if (params_normalized[params_count - 1] > 0.0f)
  {
  ...
  /* allocate qmatrix */
  qmatrix = (float **)malloc(states*sizeof(float *));
  if (!qmatrix)
  {
  ...
  success = 1;
  for (i = 0; i < states; ++i)
    if (!(qmatrix[i] = (float *)malloc(states*sizeof(float)))) success=0;
  ...
  for (i = 0; i < states; ++i)
  {
    ...
    {
      float factor = (freqs[i] <= PLL_EIGEN_MINFREQ ||
                       freqs[j] <= PLL_EIGEN_MINFREQ) ? 0 : params_normalized[k];
      k++;
      qmatrix[i][j] = qmatrix[j][i] = factor * sqrtf(freqs[i] * freqs[j]);
...
static unsigned int eliminate_zero_states(float **mat, float *forg,
                                          unsigned int states, float *new_forg)
{
                                ...
                                unsigned int params_index)
{
  ...
  float * eigenvecs = partition->eigenvecs[params_index];
  float * inv_eigenvecs = partition->inv_eigenvecs[params_index];
  float * eigenvals = partition->eigenvals[params_index];
  float * freqs = partition->frequencies[params_index];
  float * subst_params = partition->subst_params[params_index];
  ...
  d = (float *)malloc(states*sizeof(float));
  e = (float *)malloc(states*sizeof(float));
  new_freqs = (float *)malloc(states*sizeof(float));
  if (!d || !e || !new_freqs)
  {
    if (d) free(d);
    if (e) free(e);
  ...
  for (i = 0; i < new_states; i++)
    new_freqs[i] = sqrtf(new_freqs[i]);

  if (new_states < states)
  {
    /* initialize eigenvecs and inv_eigenvecs with diagonal matrix */
    memset(eigenvecs, 0, states_padded*states*sizeof(float));
    memset(inv_eigenvecs, 0, states_padded*states*sizeof(float));

    for (i = 0; i < states; i++)
    {
      eigenvecs[i*states_padded+i] = 1.f;
      inv_eigenvecs[i*states_padded+i] = 1.f;
    }
...
PLL_EXPORT void pll_set_category_rates(pll_partition_t * partition,
                                       const float * rates)
{
  memcpy(partition->rates, rates, partition->rate_cats*sizeof(float));
}

PLL_EXPORT void pll_set_category_weights(pll_partition_t * partition,
                                         const float * rate_weights)
{
  memcpy(partition->rate_weights, rate_weights,
         partition->rate_cats*sizeof(float));
}
                                                     ...
                                                     float prop_invar)
{
  ...
  if (prop_invar != 0.0f && (partition->attributes & PLL_ATTRIB_AB_MASK))
  {
  ...
  if (prop_invar > 0.0f && !partition->invariant)
  {
