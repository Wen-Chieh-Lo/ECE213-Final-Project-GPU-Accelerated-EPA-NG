...
static void set_missing_branch_length(pll_rtree_t * tree, float length)
{
  unsigned int i;
...
int main(int argc, char * argv[])
{
  ...
  /* fix all missing branch lengths (i.e. those that did not appear in the
     newick) to 0.000001 */
  set_missing_branch_length(tree, 0.000001f);
  ...
  */
  ...
  branch_lengths = (float *)xmalloc(branch_count * sizeof(float));
  ...
  float frequencies[4] = { 0.299f, 0.183f, 0.196f, 0.322f };
  ...
  float subst_params[6] = {0.28468f,1.01818f,0.11409f,0.19313f,3.46379f,1.00000f};
  ...
  */
  ...
  float logl = pll_compute_root_loglikelihood(partition,
                                               tree->root->clv_index,
